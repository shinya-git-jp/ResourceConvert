...
const ErrorMessageDisplay: React.FC = () => {
    ...
    async (
      ...
      try {
        const response = await fetch(
          "http://localhost:8080/api/error-messages/fetch",
          {
            method: "POST",
  ...
  const handleNavigateToConvert = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:8080/api/error-messages/fetch/by-ids",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(requestBody),
      ...
      navigate("/error-messages-xml", {
        state: { messages: selectedMessagesData, languageMap: langMap },
      });
    } catch (error: any) {
  ...
  const handleSelectPage = () => {
    const currentPageObjectIDs = messages.map((m) => m.objectID);
    setSelectedObjectIDs((prevSet) => {
      const newSet = new Set(prevSet);
  ...
  const handleSelectAllFiltered = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:8080/api/error-messages/fetch/ids",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(requestBody),
                  ...
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell sx={{ minWidth: 200 }}>
                        {langMap?.country1 && langMap.country1.trim() !== ""
                          ? `${langMap.country1} (Country1)`
                          : "Country1"}
                      </TableCell>
                      <TableCell sx={{ minWidth: 200 }}>
                        {langMap?.country2 && langMap.country2.trim() !== ""
                          ? `${langMap.country2} (Country2)`
                          : "Country2"}
                      </TableCell>
                      <TableCell sx={{ minWidth: 200 }}>
                        {langMap?.country3 && langMap.country3.trim() !== ""
                          ? `${langMap.country3} (Country3)`
                          : "Country3"}
                      </TableCell>
                      <TableCell sx={{ minWidth: 200 }}>
                        {langMap?.country4 && langMap.country4.trim() !== ""
                          ? `${langMap.country4} (Country4)`
                          : "Country4"}
                      </TableCell>
                      <TableCell sx={{ minWidth: 200 }}>
                        {langMap?.country5 && langMap.country5.trim() !== ""
                          ? `${langMap.country5} (Country5)`
                          : "Country5"}
                      </TableCell>
                    </TableRow>
                  </TableHead>
