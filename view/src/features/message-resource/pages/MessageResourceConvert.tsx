...
export const MessageResourceConvert = () => {
  ...
  const [selectedCountryForPreview, setSelectedCountryForPreview] =
    useState<keyof LanguageMap>("country1");
  const previewTextareaRef = useRef<HTMLTextAreaElement>(null);

  const [isModalOpen, setIsModalOpen] = useState(false);
  const [downloadLangs, setDownloadLangs] = useState<Array<keyof LanguageMap>>([
    selectedCountryForPreview,
  ]);
  const [downloadFilename, setDownloadFilename] = useState("");

  const availableLangKeys = (
    ["country1", "country2", "country3", "country4", "country5"] as Array<
      keyof LanguageMap
    >
  ).filter(
    (key) =>
      (langMap && langMap[key] && langMap[key].trim() !== "") ||
      key === "country1",
  );
  ...
  const executeDownload = async () => {
    ...
    if (downloadLangs.length === 1) {
      ...
      const finalFilename = downloadFilename.endsWith(".properties")
        ? downloadFilename
        : downloadFilename + ".properties";
      ...
      downloadLangs.forEach((countryKey) => {
        const langName =
          langMap && langMap[countryKey]?.trim()
            ? langMap[countryKey]
            : countryKey;
        const filenameInZip =
        ...
        zip.file(
          filenameInZip,
          new Blob([bom, content], { type: "text/plain;charset=utf-8" }),
        );
      });

      try {
        const zipBlob = await zip.generateAsync({ type: "blob" });
        const finalFilename = downloadFilename.endsWith(".zip")
          ? downloadFilename
  ...
  const openDownloadModal = () => {
    if (downloadLangs.length === 1) {
      ...
      const previewLangName =
        langMap && langMap[previewLangKey]?.trim()
          ? langMap[previewLangKey]
          : previewLangKey;
      setDownloadFilename(
        previewLangName && previewLangName.trim() !== ""
          ? `output_${previewLangName}.properties`
          : "output.properties",
      );
  ...
  useEffect(() => {
    ...
    const langName =
      langMap && langMap[selectedCountryForPreview]?.trim()
        ? langMap[selectedCountryForPreview]
        : selectedCountryForPreview;
    setDownloadFilename(
      langName && langName.trim() !== ""
        ? `output_${langName}.properties`
          ...
          <FormControl sx={{ minWidth: 200 }} size="small">
            ...
            <Select
              ...
              onChange={(e: SelectChangeEvent<string>) =>
                setSelectedCountryForPreview(
                  e.target.value as keyof LanguageMap,
                )
              }
            >
