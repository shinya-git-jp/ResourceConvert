...
const DisplayStateContext = createContext<DisplayStateContextType | undefined>(
  undefined,
);
...
export const DisplayStateProvider: React.FC<DisplayStateProviderProps> = ({
  children,
}) => {
  const [selectedConfigName, setSelectedConfigName] = useState<string>("");
  const [messageSelectedObjectIDs, setMessageSelectedObjectIDs] = useState(
    new Set<string>(),
  );
  const [errorMessageSelectedObjectIDs, setErrorMessageSelectedObjectIDs] =
...
export const useDisplayState = (): DisplayStateContextType => {
  ...
  if (context === undefined) {
    throw new Error(
      "useDisplayState must be used within a DisplayStateProvider",
    );
  }
  return context;
};
